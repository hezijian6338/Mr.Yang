...
public class SkuListController {
    ...
    private final SkuListService service;

    @Autowired
    public SkuListController(SkuListService service) {
        this.service = service;
    }

    @RequestMapping(value = "{id}", method = RequestMethod.GET)
    public Result get(@PathVariable("id") String id) {
        SkuList skuList = service.findById(id);
        return new Result<>(skuList);
    }
    ...
    public Result add(@RequestBody SkuList skuList) {
        // skuList.setImg(file);
        return new Result<>(service.add(skuList));
    }
