...
@RestController
@RequestMapping("goodsPromise")
@Api("GoodsPromise操作Api")
public class GoodsPromiseController {

    private final GoodsPromiseService service;

    @Autowired
    public GoodsPromiseController(GoodsPromiseService service) {
        this.service = service;
    }

    @RequestMapping(value = "{id}", method = RequestMethod.GET)
    public Result get(@PathVariable("id") String id) {
        GoodsPromise goodsPromise = service.findById(id);
        return new Result<>(goodsPromise);
    }
    ...
    public Result add(@RequestBody GoodsPromise goodsPromise) {
        // goodsPromise.setImg(file);
        return new Result<>(service.add(goodsPromise));
    }
